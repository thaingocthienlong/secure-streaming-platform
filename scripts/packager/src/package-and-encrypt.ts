...
async function packageVideo(courseCode: string, videoId: string, inputPath: string) {
  ...
  const kid    = Plaintext.slice(0, 16).toString('hex')
  ...
  const dashArgs = [
    // Video stream with HD label
    `in=${inputPath},stream=video,output=${path.join(dashOut,'video_init.mp4')},segment_template=${path.join(dashOut,'video_seg-$Number$.m4s')},drm_label=HD`,
    // Audio stream with AUDIO label  
    `in=${inputPath},stream=audio,output=${path.join(dashOut,'audio_init.mp4')},segment_template=${path.join(dashOut,'audio_seg-$Number$.m4s')},drm_label=AUDIO`,
    ...
    '--clear_lead', '0', // Important: No clear lead
  ]
  ...
  const hlsArgs = [
    ...
    `in=${inputPath},stream=video,playlist_name=video.m3u8,segment_template=${path.join(hlsOut,'video_seg-$Number$.ts')},drm_label=HD`,
    // Audio stream with AUDIO label - HLS uses playlist_name instead of output for playlists
    `in=${inputPath},stream=audio,playlist_name=audio.m3u8,segment_template=${path.join(hlsOut,'audio_seg-$Number$.ts')},drm_label=AUDIO`,
